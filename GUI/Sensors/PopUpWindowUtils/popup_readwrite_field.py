...
from GUI.Sensors.PopUpWindowUtils.validators import (
    ValidatorAggregate,
    ValidatorInt,
    ValidatorFloat,
)
...
class ReadWriteField(Field):
    def __init__(
        self, field_name: str, set_function=lambda x: ..., default_value="", **kwargs
    ):
        ...
        self.value_input = TextInput(
            text=str(self.default_value),
            multiline=False,
            size_hint=(None, 1),
            size=(self.size[0] / 2 - 50, 0),
        )
...
class ReadWriteFieldInt(ReadWriteField):
    def __init__(
        self, field_name: str, set_function, min_val: int, max_val: int, **kwargs
    ):
