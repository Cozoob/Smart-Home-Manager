...
from Sensors.sensors import HumidSensor


class SchemeHumidSensor(SchemeSensor):

    MAIN_IMAGE = "./Resources/SensorIcons/humid-icon.jpeg"
    SENSOR_NAME = "Humid"
    REFRESH_RATE_SECONDS = 1

    def __init__(self, sensor:HumidSensor, **kwargs):
        super(SchemeHumidSensor, self).__init__(self.SENSOR_NAME, sensor, **kwargs)
        self.sensor = sensor
    ...
    def get_popup_window_content(self) -> Tuple[Layout, Button]:
        ...
        def update(*args):
            nonlocal humid_value
            humid_value.update_value(self.sensor.get_humid())
            pass
        ...
        # todo
        humid_value = ReadOnlyField("Humid: ")
        main_boxlayout.add_widget(humid_value)
